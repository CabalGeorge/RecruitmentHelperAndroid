...
public class ArchivedAdapter extends RecyclerView.Adapter<ArchivedAdapter.ViewHolder> {
   ...
   @NonNull
   @Override
   public ArchivedAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

      View view = inflater.inflate(R.layout.custom_candidate_layout, parent, false);
      return new ArchivedAdapter.ViewHolder(view);
   }

   @Override
   public void onBindViewHolder(@NonNull ArchivedAdapter.ViewHolder holder, int position) {
   ...
   public class ViewHolder extends RecyclerView.ViewHolder implements View.OnCreateContextMenuListener {
      ...
      public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
         ...
         menu.add(this.getAdapterPosition(), 1, 1, "Unarchive this candidate");
   ...
   public void unarchiveCandidate(int position) {
      RequestQueue requestQueue = Volley.newRequestQueue(cardView.getContext());

      String urlDelete = Constant.ARCHIVE_CANDIDATE_URL + "/" + candidates.get(position).getCandidateId() + "/unarchive";
      StringRequest stringRequest = new StringRequest(Request.Method.PATCH, urlDelete, response -> {
         Toast.makeText(cardView.getContext(), "Candidate successfully unarchived!", Toast.LENGTH_LONG).show();
      }, error -> Toast.makeText(cardView.getContext(), "error" + error.toString(), Toast.LENGTH_LONG).show()) {};
